...
```python
from training.training import Experiment, TrainingConfig
...
```bash
TRN_CONFIG_FILE=training.yaml python -c "from training.training.config import load_training_config; c = load_training_config(); print(c.train.epochs)"
```

Generate a template:

```python
from training.training.config import save_training_template
...
```bash
python -m pytest training/training -q
```
