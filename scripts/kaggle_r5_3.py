"""R5.3.1 — Automated Local -> Kaggle CLI Training Deployment.
...
NOTEBOOK_SRC = REPO_ROOT / "training" / "kaggle" / "notebooks" / "R5_3_train.ipynb"
DEPLOY_DIR = REPO_ROOT / "training" / "kaggle" / "deployment" / "r5_3"
...
OUTPUT_DIR = REPO_ROOT / "artifacts" / "r5_3"
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for k in kernels:
                title = k.get("title", "")
                ref = k.get("ref", "")
                # Match R5.3 training kernel by title
                if "r5.3" in title.lower() and "multimodal" in title.lower():
                    return ref
            # Fallback: match by slug pattern
            for k in kernels:
                ref = k.get("ref", "")
                if "r5-3" in ref and "multimodal" in ref:
                    return ref
    except Exception:
        pass
    # Fallback to expected slug
    return f"{username}/r5-3-cropfusion-multimodal-training"
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    _print_section("R5.3.1 LOCAL PRE-FLIGHT CHECK")
...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    # Copy notebook
    dst_nb = DEPLOY_DIR / "R5_3_train.ipynb"
    ...
    metadata = {
        "id": kernel_id,
        "title": "R5.3 CropFusion Multimodal Training",
        "code_file": "R5_3_train.ipynb",
        "language": "python",
    ...
    for fname in ["R5_3_train.ipynb", "kernel-metadata.json"]:
...
def cmd_train(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "confirm", False):
        ...
        _print_row("Kernel ID", kernel_id)
        _print_row("Estimated time", "30-60 minutes")
        print()
...
def cmd_full(args: argparse.Namespace) -> int:
    ...
    _print_section("R5.3.1 FULL DEPLOYMENT PIPELINE")
    ...
    _print_section("R5.3.1 COMPLETE — RESULTS READY FOR REVIEW")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="kaggle_r5_3",
        description="R5.3.1 Automated Kaggle Training Deployment",
    )
    parser.add_argument(
        "--timeout",
        type=int,
